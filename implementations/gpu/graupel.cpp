...
void graupel(size_t nvec, size_t ke, size_t ivstart, size_t ivend,
  ...
  const size_t GRID_SIZE = (nvec + BLOCK_SIZE - 1) / BLOCK_SIZE;
...
#pragma omp teams num_teams(GRID_SIZE) thread_limit(BLOCK_SIZE)
            ...
            for (size_t k = pipeline; k < pipeline_end; k++) {
              ...
              if (k < ke - 1) {
                ...
                if ((qc_qmin or qr_qmin or qs_qmin or qi_qmin or qg_qmin) or
                  ...
                  solidify(nexted_vec_index, is_sig_present, dt, dz_d, t_d,
                           rho_d, p_d, qv_d, qc_d, qi_d, qr_d, qs_d, qg_d, qnc);
                }
              }
              ...
              if (internals[iv].kmin_flag) {
                ...
                if (internals[iv].kmin_flag & 0b1000) {
                  real_t vc = vel_scale_factor(0, xrho, rho_d[oned_vec_index],
                                               t_d[oned_vec_index],
                                               qr_d[oned_vec_index]);
                  precip<0>(qr_d[oned_vec_index], internals[iv].prr_gsp,
                            internals[iv].vt[0], zeta, vc,
                            internals[iv].prr_gsp, internals[iv].vt[0],
                            qr_d[oned_vec_index], qr_d[kp1 * ivend + iv],
                            rho_d[oned_vec_index]);
                }

                // ix = 1, qp_ind[1] = 1
                if (internals[iv].kmin_flag & 0b0100) {
                  real_t vc = vel_scale_factor(1, xrho, rho_d[oned_vec_index],
                                               t_d[oned_vec_index],
                                               qi_d[oned_vec_index]);
                  precip<1>(qi_d[oned_vec_index], internals[iv].pri_gsp,
                            internals[iv].vt[1], zeta, vc,
                            internals[iv].pri_gsp, internals[iv].vt[1],
                            qi_d[oned_vec_index], qi_d[kp1 * ivend + iv],
                            rho_d[oned_vec_index]);
                }

                // ix = 2, qp_ind[2] = 2
                if (internals[iv].kmin_flag & 0b0010) {
                  real_t vc = vel_scale_factor(2, xrho, rho_d[oned_vec_index],
                                               t_d[oned_vec_index],
                                               qs_d[oned_vec_index]);
                  precip<2>(qs_d[oned_vec_index], internals[iv].prs_gsp,
                            internals[iv].vt[2], zeta, vc,
                            internals[iv].prs_gsp, internals[iv].vt[2],
                            qs_d[oned_vec_index], qs_d[kp1 * ivend + iv],
                            rho_d[oned_vec_index]);
                }

                // ix = 3, qp_ind[3] = 3
                if (internals[iv].kmin_flag & 0b0001) {
                  real_t vc = vel_scale_factor(3, xrho, rho_d[oned_vec_index],
                                               t_d[oned_vec_index],
                                               qg_d[oned_vec_index]);
                  precip<3>(qg_d[oned_vec_index], internals[iv].prg_gsp,
                            internals[iv].vt[3], zeta, vc,
                            internals[iv].prg_gsp, internals[iv].vt[3],
                            qg_d[oned_vec_index], qg_d[kp1 * ivend + iv],
                            rho_d[oned_vec_index]);
                }
                ...
                internals[iv].eflx =
                    dt * (internals[iv].prr_gsp *
                              (clw * t_d[oned_vec_index] -
